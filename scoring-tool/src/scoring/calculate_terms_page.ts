...
export async function calculateTermsPage({ baseUrl, playwrightPage, puppeteerPage }: Params) {
  const flow = await startFlow(puppeteerPage);

  consola.debug("TermsPage - navigate");
  ...
  consola.debug("TermsPage - navigate end");
  const {
    steps: [result],
