...
test.describe("DM一覧", () => {
  ...
  test("新規DM開始モーダルが初期仕様通りにバリデーションされること", async ({
    page,
  }) => {
    await login(page);
    await page.goto("/dm");
    ...
    await expect(page).toHaveScreenshot(
      "dm-新規DM開始モーダル（存在しないユーザー名）.png",
    );
  });

  test("送信ボタンをクリックすると、DM詳細画面に遷移すること", async ({
    page,
  }) => {
    await login(page);
    await page.goto("/dm");
  ...
  test("Enterでメッセージを送信・Shift+Enterで改行できること", async ({
    page,
  }) => {
  ...
  test("相手が入力中の場合、入力中のインジケータが表示されること", async ({
    page,
    browser,
  }) => {
    await login(page, "gg3i6j6");
    ...
    await expect(page.getByText("入力中…")).not.toBeVisible({
      timeout: 10 * 1000,
    });
  ...
  test("メッセージ・既読がリアルタイムで更新されること", async ({
    page,
    browser,
  }) => {
    await login(page, "gg3i6j6");
    ...
    const pageLastMessage = page
      .getByTestId("dm-message-list")
      .locator("li")
      .last();
    const peerLastMessage = peerPage
      .getByTestId("dm-message-list")
      .locator("li")
      .last();
