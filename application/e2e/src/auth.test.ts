...
test.describe("サインイン・新規登録", () => {
  ...
  test("新規登録ができる", async ({ page }) => {
    ...
    await page
      ...
      .pressSequentially("テストユーザー");
    await page
      .getByRole("textbox", { name: "パスワード" })
      .pressSequentially("testpass-123");
  ...
  test("既に使われているユーザー名で登録するとエラーが表示される", async ({
    page,
  }) => {
    await page.getByRole("button", { name: "初めての方はこちら" }).click();
    await page
    ...
    // エラーメッセージが表示される
    await expect(page.getByText("ユーザー名が使われています")).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test("サインインに失敗するとエラーが表示される", async ({ page }) => {
    ...
    // エラーメッセージが表示される
    await expect(page.getByText("パスワードが異なります")).toBeVisible({
      timeout: 10_000,
    });
  });
});
